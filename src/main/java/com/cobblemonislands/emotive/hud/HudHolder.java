...
public class HudHolder extends ElementHolder {
    ...
    public HudHolder(ServerPlayer player, List<String> availableAnimations) {
        super();
...
//            this.labels.add(label);
            ...
            var renderer = new HudPlayerModelHolder(player.serverLevel(), player, PlayerModelRegistry.getTexturedModel(PlayerModelRegistry.getAnimations().get(i)));
        ...
        this.modelAttachments.forEach(holder -> {
            for (Bone bone : holder.getBones()) {
                ...
                holder.applyPose(bone.getDefaultPose(), bone);
            }
            holder.asyncTick();
        });
        this.slotChanged(0);
    ...
    protected void select() {
        try {
            var animation = ModConfig.getInstance().getAnimation(PlayerModelRegistry.getAnimations().get(this.slot));
            if (animation != null)
                GestureController.onStart(player, animation.getSecond(), GestureController.getOrCreateModelData(player).get());
